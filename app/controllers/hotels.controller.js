...
exports.findAll = (req, res) => {
  ...
  var condition = hotel_name ? { hotel_name: { $regex: new RegExp(hotel_name), $options: "i" } } : {};
  ...
  Hotel.find(condition)
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while retrieving Hotelss."
      });
    });
...
exports.findbyname=(req,res)=>{
  ...
  .then(data => {
    if (!data)
      res.status(404).send({ message: "Not found Hotels with id " + Hotel_name });
    else res.send(data);
  })
  .catch(err => {
    res
